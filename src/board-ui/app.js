...
async function renderMermaid() { if (!window.mermaid) return; const nodes = Array.from(document.querySelectorAll('.mermaid')); if (!nodes.length) return; try { window.mermaid.initialize({ startOnLoad: false, securityLevel: 'strict', theme: lightTheme() ? 'default' : 'dark' }); await window.mermaid.run({ nodes }); } catch (error) { message(`Mermaid 렌더링 실패: ${error.message}`, true); } }
...
document.addEventListener('change', (event) => { const input = event.target.closest('[data-task-field]'); if (!input) return; const task = state.snapshot.tasks.tasks.find((item) => item.id === state.selected); if (!task) return; queueTaskUpdate(task, { [input.dataset.taskField]: input.value || null }); });
...
el('new-task').addEventListener('click', () => { el('task-id').textContent = 'NEW TASK'; el('task-title').value = ''; el('task-summary').value = ''; el('task-acceptance').value = ''; el('task-links').value = ''; el('task-dialog').showModal(); el('task-title').focus(); });
