...
function readConfig(file) {
  ...
  for (const field of Object.keys(value)) if (!['schemaVersion', 'documentTypes', 'documentStates'].includes(field)) throw new Error(`${file}: 지원하지 않는 필드입니다: ${field}`);
  if (value.schemaVersion !== 1) throw new Error(`${file}: 지원하지 않는 board.json schemaVersion입니다: ${value.schemaVersion}`);
  for (const [group, keys] of [['documentTypes', DOCUMENT_TYPE_KEYS], ['documentStates', DOCUMENT_STATE_KEYS]]) {
...
function mergePresentation(target, source) {
  ...
  for (const group of ['documentTypes', 'documentStates']) {
...
function renderProjectBoardConfig() {
  return `${JSON.stringify({ schemaVersion: 1, documentTypes: {}, documentStates: {} }, null, 2)}\n`;
}

module.exports = {
  DOCUMENT_TYPE_KEYS, DOCUMENT_STATE_KEYS, DEFAULT_PRESENTATION,
