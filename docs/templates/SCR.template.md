...
이 화면에서 다른 화면으로 나가는 이동만 적는다. 같은 화면에 머무는 표시 변화는 아래 `상태` 표가 정본이다.

| 트리거 | 조건 | 대상 |
|---|---|---|
| <사용자 행동 또는 시스템 이벤트> | <이동을 가르는 판정> | `[[SCR-001-<대상-화면명>\|SCR-001]]` |

아래 다이어그램은 위 표에서 파생한 보조 뷰다. 트리거, 조건과 대상의 정본은 표이며 두 표현이 어긋나면 표를 따른다.

```mermaid
flowchart LR
    SCR-000["<이 화면>"] -->|<조건>| SCR-001["<대상 화면>"]
```
