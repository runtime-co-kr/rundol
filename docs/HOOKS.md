...
| **4** | Git `pre-commit`·`commit-msg` — 훅이 꺼져도 서는 층 | 안착 경로가 막히지 않는다 | **완료** |

### 4단계가 다른 층인 이유

1~3단계는 전부 AI 클라이언트 안에서 돈다. `disableAllHooks` 한 줄로 꺼지고, 사람이 터미널에서 치는 커밋에는 아무것도 걸리지 않는다. 그 통제에는 **사람 모양의 구멍**이 남는다. 사람 워커와 에이전트 워커가 같은 계층이라는 것이 이 프로젝트의 전제라면, 같은 경계를 지나야 한다.

| 훅 | 보는 것 | 막는 것 |
|---|---|---|
| `pre-commit` | 브랜치 + 스테이지된 경로 | 기본 코드 브랜치에 제품 코드 |
| `commit-msg` | 커밋 메시지 | `Rundol-Task` 없는 제품 코드 커밋 |

둘로 나눈 것은 필요한 값이 다르기 때문이다. 한 훅에 몰면 막힌 이유가 브랜치인지 결박인지 사유 한 줄로 갈리지 않는다.

**안착 경로는 열려 있다.** git은 `git merge`에 `pre-commit`을 돌리지 않고, `commit-msg`는 `Merge`·`Revert`로 시작하는 메시지에 결박을 묻지 않는다. fast-forward와 병합 커밋 둘 다 지난다 — 막기만 하고 나갈 길이 없으면 경계가 아니라 벽이다.

우회는 `--no-verify`로 열려 있고 막지 않는다. 기본값을 뒤집는 것이 목적이지 손을 묶는 것이 목적이 아니다.
...
`rdl skill install`이 `SessionStart` 구간 하나를 **병합한다.** 덮어쓰지 않는다. 나머지 이벤트는 아래 스니펫을 각자 복사한다.

```
rdl skill install [--no-hooks]   스킬을 놓고 SessionStart 구간을 병합한다
rdl skill hooks                  런돌이 넣은 구간과 남아 있는 사용자 항목을 보인다
rdl skill hooks --remove         런돌 구간만 뺀다
```

`SessionStart`만 자동으로 넣는 이유는 그 자리가 **조회만 하기 때문**이다. 막는 훅(`Stop`, `PreToolUse`)을 도구가 남의 설정에 넣으면, 설치 한 번으로 그 사람의 작업이 막히기 시작한다. 막는 층은 사람이 읽고 놓는다.

병합의 규율은 셋이다.

- **표시.** 런돌이 넣은 항목에는 `"rundolManaged": "rundol/session-start"`가 붙는다. 표가 없는 항목은 사용자의 것이며 읽지도 옮기지도 지우지도 않는다. 별도 장부 파일을 두지 않는 이유는 장부가 설정과 갈리기 때문이다 — 사람이 항목을 손으로 지우면 장부는 여전히 "넣었다"고 말한다.
- **보존.** 다른 이벤트, 같은 이벤트의 다른 항목(**순서까지**), 최상위의 다른 열쇠가 전부 그대로 남는다. 되돌리면 내용이 원본과 같아진다. JSON을 다시 직렬화하므로 들여쓰기는 정규화될 수 있으나 값은 하나도 달라지지 않는다.
- **드러냄.** 읽지 못하거나 모양이 다르면 그 파일을 **건드리지 않고 종료 상태 2로 끝낸다.** 조용히 덮어쓰지도, 건너뛰고 성공이라 말하지도 않는다. 건너뛰면 사람은 훅이 설치된 줄 알고, 설치되지 않은 훅은 없는 훅이며, 없는 훅은 아무 신호 없이 꺼진 통제와 구분되지 않는다. 한 대상이 실패해도 다른 대상은 시도된다 — 첫 실패에서 멈추면 무엇이 설치됐고 무엇이 안 됐는지에 답할 수 없다.

표 없는 `rdl hook` 호출이 이미 있으면 그 자리는 **보존하고 넣지 않는다.** 넣으면 세션마다 훅이 두 번 돌고 컨텍스트가 두 벌 주입된다. `rdl skill install`이 표 없는 스킬 디렉터리를 사용자 소유로 보고 보존하는 것과 같은 판단이다.

Copilot은 대상이 아니다. 스킬은 파일을 놓는 일이라 형식이 필요 없지만 훅은 형식이 있어야 하고, 없는 형식을 지어내면 그 파일은 아무도 읽지 않는다.
...
Rundol CLI가 자동으로 넣는 것은 `SessionStart` 구간 **하나**다. 그 자리는 조회만 하고 막지 않으므로, 잘못 들어가도 사람의 작업을 멈추지 않는다. 막는 훅과 OS 유닛은 그보다 강한 개입이므로 이 문서의 스니펫을 사람이 읽고 직접 놓는다 — 강한 개입은 명시적 행위로만 놓인다는 같은 규율이다.
