...
Rundol CLI의 기본 명령은 `rdl`이며 `rundol`은 같은 실행 파일의 별칭이다. Node.js 20 이상과 Git에서 동작한다. 판정과 저장은 외부 runtime dependency 없이 돌고, 보드 화면과 그림 처리에 쓰는 `mermaid`, `marked`, `dompurify`, `jpeg-js`만 `@rundol/cli`가 함께 설치한다.
...
`sessions`는 이 작업공간에 지금 붙어 있는 세션입니다. 런과 같은 물음의 다른 축이며, 무엇을 해야 하는가 옆에 누가 같이 있는가가 있어야 합니다. 사람이 읽는 출력에서는 **둘 이상일 때만** `동시세션` 줄로 나오고 각자의 작업 공간 경로를 함께 냅니다 — 같은 트리에 있는 세션과 나뉘어 있는 세션은 위험이 다르고, 경로가 그 차이를 드러내는 유일한 값입니다.

세션을 세지 못한 것은 혼자인 것과 다릅니다. 읽기에 실패하면 `--json`은 `sessionsUnreadable`에 그 사유를 싣고, 사람이 읽는 출력은 `동시세션 확인불가 :: <사유>` 줄을 냅니다. 실패를 빈 목록으로 접으면 "혼자"와 "못 셌다"가 같은 값이 되고, 둘 이상일 때만 말하는 규칙과 겹쳐 세션을 읽지 못하는 저장소는 영영 조용해집니다 — 경고가 필요한 바로 그때 침묵하는 방향입니다. 세션을 읽지 못해도 런의 답은 그대로 냅니다.

읽기만 합니다. 등록은 `rdl session start`가 하고 여기서는 잠금 파일을 읽을 뿐입니다. 조회가 등록을 겸하면 무엇이 주의를 요구하는지 묻는 행위가 상태를 바꿉니다. 그리고 Rundol 작업공간을 찾지 못해도 세션은 답합니다 — 세션 worktree에는 `projects/`가 없으므로 여기서 접으면 정작 격리된 세션이 "나 말고 누가 있나"를 물을 수 없습니다.
...
| `rdl rule` | 데이터로 정의된 규칙의 발화 이력, 우회와 사유, 죽은 규칙 조회 | `firing` 원장 샤드 | 없음 |
...
커밋 시점의 경계도 함께 세운다. `pre-commit`은 기본 코드 브랜치에서 제품 코드를 담는 것을 거절하고, `commit-msg`는 제품 코드 커밋에 `Rundol-Task` 트레일러를 요구한다.

여기서 제품 코드는 실행되는 코드만이 아니다. `src/` `bin/` `packages/` `scripts/`에 더해 `skills/` `.github/` `test/`가 대상이다. 스킬은 `rdl skill install`이 세 클라이언트의 홈에 복사하므로 한 줄이 그 저장소를 쓰는 모든 에이전트의 판단을 바꾸고, 배포 워크플로는 무엇이 언제 npm에 올라가는지를 정하며, 시험은 통제가 실제로 서 있는지를 판정하는 층이다 — 여기가 자유로우면 통제를 지우는 가장 조용한 길이 검사를 고치는 것이 된다. `docs/`는 대상이 아니다. 이 목록은 Rundol을 쓰는 모든 프로젝트에 실려 나가는 기본값이고, 남의 저장소에서 문서 한 줄 고치는 데 브랜치를 요구하는 것은 이 도구가 정할 일이 아니다. 두 훅으로 나눈 것은 필요한 값이 다르기 때문이다 — 브랜치와 경로는 메시지 없이 판정할 수 있지만 결박은 메시지가 있어야 보인다. 기존 훅은 `<이름>.rundol-user`로 보존해 먼저 실행한다.
...
`start`는 세션을 등록한다. 등록은 새 저장 형식을 만들지 않고 런타임 잠금 디렉터리를 쓴다 — 그 자리가 이미 "누가 무엇을 쥐고 있는가"를 pid와 함께 적는 곳이고, 세션 등록이 묻는 것도 같은 물음이다. 잠금을 놓지 않고 남기므로 세션은 명령보다 오래 살고, 그 세션이 죽으면 pid 생존 확인이 대신 회수한다.

생존을 말해 줄 pid는 `RUNDOL_SESSION_PID`, `CLAUDE_PID` 순으로 받는다. `rdl`은 명령마다 죽으므로 자기 pid로는 아무것도 말할 수 없다 — 호스트가 알려주는 클라이언트 pid라야 "이 세션이 지금 붙어 있는가"가 성립한다. 없으면 등록하지 않고 `registerSkipped`로 그 사실을 낸다.

`list`의 `alive`는 셋을 구분한다. `true`는 붙어 있는 것, `false`는 등록했다가 끝난 것, `null`은 등록을 지나지 않은 것이다. 하나로 접으면 worktree만 만들고 등록하지 않은 세션이 죽은 세션으로 읽힌다.

`list`는 이 저장소의 **모든** 작업 트리를 낸다. `rdl` 밖에서 만든 것도 포함한다 — 이 명령이 답하는 물음은 "지금 이 저장소에 누가 붙어 있는가"이고, 자기가 만든 것만 세면 그 물음에 답하지 못한다. 세션이 연 자리인지는 `managed`가 가른다. 본 작업 트리와 `rundol/*` 브랜치의 worktree는 빼는데, 그것은 붙어 있는 누군가가 아니라 저장소의 구조이고 늘 있으므로 세면 답이 언제나 둘 이상이 되어 "혼자인가"를 물을 수 없게 된다.
...
작업 공간은 저장소 안의 `.rundol/worktrees/<id8>`에 만든다. 추적 제외는 제품 브랜치의 `.gitignore`가 들고 오므로 clone과 함께 규칙이 온다 — 규칙을 빠뜨린 창이 없어야 안에 둘 수 있고, 그래서 `rdl hook session-start`가 세션을 열기 전에 두 규칙의 존재를 확인하고 없으면 채운다. `--path`로 자리를 지정할 수 있다.
...
rdl rule history --project memo --rule done-requires-test-link
rdl rule bypasses --project memo
rdl rule dead --project memo
```

어휘를 데이터로 여는 설계가 이 계측을 필수 부품으로 만들었다. 닫힌 어휘의 제약은 코드를 고쳐야 죽으므로 리뷰가 한 번은 본다. 열린 어휘의 제약은 설정을 잘못 고치는 것만으로 죽고, 그것을 보는 눈은 없다 — 유형 정의에서 제약 하나를 지우거나 조건을 아무것에도 맞지 않게 적으면 그 규칙은 그날부터 아무 일도 하지 않는데, 겉으로는 규칙이 있는 저장소와 똑같아 보인다.

세는 축은 둘이다. `evaluated`는 판정이 실제로 본 규칙이고 `blocked`는 그중 막은 규칙이다. 막은 것만 적으면 "한 번도 안 막은 규칙"과 "한 번도 안 불린 규칙"이 같은 침묵이 되는데, 그 둘은 정반대의 뜻이다 — 앞은 다들 지키고 있다는 뜻이고 뒤는 그 규칙이 아무 항목에도 닿지 않는다는 뜻이다.

대상은 **데이터로 정의된 규칙**이다. 유형 정의가 여는 제약(`<유형>.<제약종류>`)과 게이트가 그것이며, 진단 코드 이백여 개 중 대부분은 검사기가 코드로 들고 있는 닫힌 규칙이라 이 이력의 대상이 아니다.

기록은 원장이다. 로컬 로그가 아닌 이유는 물음이 프로젝트 전체의 것이기 때문이다 — "이 규칙이 한 번도 안 불렸나"를 내 기계에서만 세면 각자 자기 침묵만 보게 되고, 침묵은 원래 아무 신호도 내지 않는다. `firing` 원장은 `LEDGERS`와 event store의 `KINDS`에 같은 이름으로 등록되어 있고 클라이언트별 샤드로 나뉘어 병합된다.

적는 자리는 하나다. 판정부가 자기가 본 것을 값으로 돌려주고, 검사가 그것을 한자리에 모아 원장에 넘긴다. 판정 지점마다 기록을 심으면 지점이 늘 때마다 계측도 함께 심어야 하고, 빠뜨렸다는 사실은 아무 신호도 내지 않는다 — 이 계측이 존재하는 이유와 똑같은 실패다.

**같은 판정은 다시 적지 않는다.** `eventId`를 판정의 내용에서 파생하므로 답이 같으면 한 건이고, 항목이 고쳐져 답이 달라지면 새 건이다. 검사는 저장할 때마다 돌고 보드가 새로 그릴 때마다 도는데 그것을 매번 새 이벤트로 적으면 원장은 저장소의 활동량만큼 자란다. 그래서 `judgments`의 뜻은 실행 횟수가 아니라 **서로 다른 판정의 수**다 — 보드 새로고침을 세면 부지런한 저장소의 규칙이 더 살아 있어 보인다.

`history`는 규칙마다 `evaluated`·`blocked`와 닿은 항목 수, 처음·마지막 시각을 낸다. `bypasses`는 우회를 사유·결정자와 함께 낸다 — 면제는 판정 자체를 돌지 않는 것이라 `evaluated`로 세지 않으며, 같은 결정은 몇 번을 보든 한 건이고 본 횟수만 `observations`로 늘어난다.

`dead`는 두 갈래를 섞지 않고 낸다. `never`는 한 번도 불린 적 없는 규칙이라 가장 강한 뜻의 죽은 규칙이고, `silent`는 불렸으나 한 번도 막지 않은 규칙이라 다들 지키고 있다는 뜻일 수도 판정이 늘 참이라는 뜻일 수도 있어 이력이 아니라 사람이 가른다. 판정이 0건이면 모든 규칙이 `never`에 나오며, 그것은 규칙이 죽었다는 뜻이 아니라 증거가 없다는 뜻이다 — `judgments`가 그 사실을 말한다.

**이 계측이 보지 못하는 것.** 전환 판정(`origin: transition`)은 판정 함수가 서면 들어오며 지금은 `item-type`만 기록된다. 셸 훅(`pre-commit`·`commit-msg`)은 `--no-verify`로 우회되고 그 사실은 여기 오지 않는다. 아직 동기화되지 않은 다른 기기의 샤드도 합쳐지기 전까지는 보이지 않는다. 그래서 "한 번도 안 불림"은 언제나 하한이며, 결과가 `measures`와 `lowerBound`로 그 사실을 함께 낸다.

여러 프로젝트를 한 번에 검사하면 적지 않는다. 판정이 어느 프로젝트의 것인지 항목 식별자만 보고는 갈리지 않고, 틀린 프로젝트의 원장에 적힌 발화는 없는 것보다 나쁘다.
...
- 문서 중심 3패널 Workspace이며 홈·문서·태스크·런·사람과 역할·설정 화면으로 나뉜다. Markdown 읽기와 검증 편집, 조치 필요, 태스크 목록과 Board, 태스크 댓글을 제공한다.
...
- 3초마다 Workspace·문서·태스크·사람·Client·Sync·계약·표현·댓글 영역별 revision을 확인해 변경된 Snapshot을 반영한다.
- API는 한 요청에서 최대 500개 태스크를 반환하고 쓰기 요청에 로컬 세션 토큰을 요구한다.
- CSP, frame 차단과 64KB 요청 제한을 적용한다.
- 런 화면은 사람을 기다리는 런·이어서 몰 수 있는 런·구동 중인 런을 `rdl run pending`과 같은 갈래 판정으로 보여 준다. 조회는 런 원장에 아무것도 쓰지 않는다.
...
- 검토 인박스의 행은 그 자리에서 펼쳐진다. 펼치면 승인본과의 차분(`승인 이후 변경` / `제출본 비교`)이 그 행 안에 서고, 그 아래에서 승인자·근거·사유를 받아 승인한다. 문서 화면을 오가지 않는다.
- 문서 화면에도 같은 자리가 있다. 인박스를 거치지 않고 문서를 열어도 같은 차분을 보고 승인할 수 있다.
- 문서 승인은 활성 human Client만 지난다. 그 owner가 프로젝트의 활성 멤버여야 하며, 판정은 `rdl doc approve`·`rdl run approve`·`rdl sync --share-unverified`가 모두 같은 함수를 쓴다. 제출(`rdl doc submit`)은 에이전트 Client도 할 수 있다 — 에이전트가 쓰고 사람이 책임지는 것이 이 도구의 협업 모형이라, 제출까지 막으면 관문이 아니라 병목이 된다.
