...
보드 UI의 전역 단축키다. 목록은 배너의 키보드 단추 또는 `?`로 연다. 입력 칸에 커서가 있는 동안 전역 키는 동작하지 않고, 그 동안 사는 것은 Esc와 Cmd/Ctrl+Enter뿐이다 — 타이핑이 내비게이션으로 새면 안 된다. 이 표는 화면의 `?` 도움말과 같은 내용이며, 정본은 코드의 SHORTCUT_ROWS다.

| 키 | 동작 |
|---|---|
| `Cmd/Ctrl + Enter` | 문서 상세나 포커스한 문서 줄에서는 검토하고 승인을 열고(커서는 사유로), 승인 폼·대화상자 폼 안에서는 제출한다 — 같은 키의 왕복이 열기와 보내기다. 반려는 단축키를 갖지 않는다 |
| `j` · `k` | 목록에서 다음·이전 줄로. 줄은 버튼이라 Enter가 곧 열기다 |
| `c` | 새 태스크 (태스크 화면에서) |
...
검토하고 승인 판과 런 승인 대화상자가 열리면 커서가 사유 칸에 선다 — 읽고, 적고, Cmd+Enter로 승인하는 한 흐름이 마우스 없이 성립한다.
