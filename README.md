...
- `0.35.x`에서 올릴 때는 `rdl lease`와 보드 임대 엔드포인트가 사라지고 구현 준비도가 저장 대신 계산으로 바뀌므로 [Rundol 0.36 마이그레이션](docs/MIGRATION-0.36.md)을 먼저 읽는다. 저장 파일을 바꾸는 명령은 없지만, 조용히 건너뛰던 준비도 위반이 드러날 수 있다.
- `0.36.x`에서 `0.37.0`으로 올릴 때는 `rdl debug summary`의 개입 계측 필드 이름이 바뀌므로 [Rundol 0.37 마이그레이션](docs/MIGRATION-0.37.md)을 읽는다. 그 값을 읽는 자동화가 없다면 할 일이 없다.
- `0.39.x`에서 `0.40.0`으로 올릴 때는 진단의 `rule` 필드가 사라지고 문서의 기능 ID 표기가 바뀌므로 [Rundol 0.40 마이그레이션](docs/MIGRATION-0.40.md)을 읽는다. 태스크 저장 파일은 바뀌지 않지만, 문서 기능 ID는 자동 이관 명령이 없어 손으로 옮긴다.
- `0.44.x`에서 `0.45.0`으로 올릴 때는 문서의 `state`가 rdl 소유가 되고 내용의 수명이 `lifecycle`로 나뉘므로 [Rundol 0.45 마이그레이션](docs/MIGRATION-0.45.md)을 읽는다. `rdl doc migrate`가 옮기며, 계획을 먼저 내어 이 이관으로 낡게 될 승인을 문서 목록과 함께 말한다. 이미 기록된 승인은 그대로 유효하다 — 리비전 계산 판이 바뀌었지만 사건마다 자기가 쓴 판을 적어 옛 기록은 옛 판으로 잰다. 다만 0.44 이하는 새 원장을 읽지 못하므로 한 저장소를 여러 판이 함께 쓴다면 전부 올린 뒤에 승인한다.
