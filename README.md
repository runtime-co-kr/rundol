...
- `0.35.x`에서 `0.36.0`으로 올릴 때는 `rdl lease`와 보드 임대 엔드포인트가 사라지고 구현 준비도가 저장 대신 계산으로 바뀌므로 [Rundol 0.36 마이그레이션](docs/MIGRATION-0.36.md)을 먼저 읽는다. 저장 파일을 바꾸는 명령은 없지만, 조용히 건너뛰던 준비도 위반이 드러날 수 있다.
