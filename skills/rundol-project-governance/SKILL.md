...
16. Use `rdl task add`, `rdl task acceptance`, and the Board for tasks. Route only the 2–4 most relevant documents into an AI coding context.
17. Before implementation, run `rdl check --strict --implementation`. Do not implement while a linked function has unresolved rules or lacks an independent TST contract. Run `rdl check <ARTIFACT-ID> --strict` immediately after authoring, then `rdl contract check --project <key>` and `rdl check --strict`. Advisory findings guide the work; checkpoint violations block save, sync, and completion.
18. Run `rdl check --structure`; treat an existing `DESIGN.md` as a manual migration candidate and never delete it automatically. Apply cleanup only after reviewing every candidate.
19. Keep branch roles strict: application code belongs to the repository root's current code branch while the repository default is discovered from `origin/HEAD` (`main`, `master`, or a custom standard); project documents and tasks belong only to `rundol/<project-key>` through its linked worktree; registries, clients, leases, and Workspace presentation belong only to `rundol/workspace`. Never edit another role's files from the current worktree.
20. Use `rdl save` and `rdl sync` for Rundol branches. Never redirect a local ref to a differently named remote ref, bypass the managed `pre-push` hook, or use force push. Run `rdl git boundary --json` again before the final sync.
