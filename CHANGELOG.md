...
- **문서 정체성이 표시에서 분리됐다.** 문서를 가리키는 키가 조율 없는 짧은 식별자(`uid`, 8자)가 되고 번호와 제목은 표시로 내려간다. 파일 이름은 그대로라 옵시디언의 위키 링크·그래프·백링크가 전부 살아 있고, 도구만 이 값으로 조인한다. 그 결과 **문서 번호 채번의 원격 조율이 사라진다** — 지금까지는 문서를 하나 만들 때마다 원격에 예약 이벤트를 밀어 넣고 충돌하면 재시도했다. 이제 번호는 로컬 최대값에서 계산되며, 예약 후 중단이나 삭제로 생기는 번호의 구멍은 정체성을 깨뜨리지 않는 표시상의 빈칸일 뿐이다. 식별자 없는 기존 문서는 `RDL-DOC-014` 경고로 드러나고 `rdl doc identity --apply`가 부여하며, 중복은 `RDL-DOC-016`으로 두 문서를 모두 지목한다. 소급 부여는 문서 내용을 바꿔 리비전이 변하므로 기존 승인이 낡는다 — `--apply` 없이는 무엇이 바뀔지만 보여준다.
