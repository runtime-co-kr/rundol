...
- **초안과 정본의 경계가 생겼다.** AI가 만든 문서는 승인 기록이 없으므로 미승인이고, 사람이 `rdl doc approve`로 그 리비전을 승인해야 정본이 된다. 승인은 내용 리비전에 결박되므로 한 글자만 고쳐도 낡음으로 바뀌고, 나중에 "그건 다른 버전이었다"가 통하지 않는다. 신뢰 상태는 저장하지 않고 파생한다 — 승인 결과를 frontmatter에 쓰면 그 쓰기가 리비전을 바꿔 자기 승인을 무효화하고, 파생이라야 AI가 `state: active`로 적어도 소용이 없다. `rdl doc status`가 승인·낡음·미승인을 일람하고, `rdl doc history`가 승인 기록과 커밋을 함께 보여주며, `rdl doc diff --since-approval`이 승인 이후 변경분만 보여준다 — 재승인이 싸야 엄격한 무효화가 유지된다. 승인 근거(직접 읽음·verdict·검사·위임)는 필수이고 사유 문장은 선택이다. 강제된 사유는 빈 문장으로 채워질 뿐, 나중에 "AI 검토가 놓쳤나 사람이 건너뛰었나"를 구분해 주지 못한다.
