...
- **조회 인덱스가 생겼다 — 삭제 가능한 캐시로, 그리고 기본값이 아닌 선택지로.** `rdl index rebuild`가 문서와 태스크의 스냅숏을 런타임 홈에 만들고 `rdl task list --index`, `rdl context --index`가 그것을 쓴다. **기본 경로는 인덱스가 아니라 정본이다.** 이 저장소에서 재 보니 정본 조회 전체가 4ms인데 인덱스를 써도 되는지 확인하는 데만 259ms가 들었다 — 비용의 전부는 지문을 만드는 Git 하위 프로세스 두 번(`rev-parse` 131ms, `status` 147ms)이다. 아끼는 것보다 확인하는 것이 비싸면 캐시는 가속이 아니라 감속이므로 기본값으로 두지 않는다. 같은 이유로 증분 갱신도 만들지 않았다 — 갱신을 아무리 싸게 해도 확인 비용이 그대로면 조회는 빨라지지 않는다. **교차점이 어디인지는 아직 모른다**: 4ms는 태스크 저장소를 읽는 비용이지 문서 전체를 읽는 비용이 아니므로 단순 외삽할 수 없고, 규모별로 다시 재야 한다. 유효성 지문은 커밋되지 않은 파일의 **내용**까지 덮는다 — 상태 목록만 보면 이미 수정된 파일을 다시 고쳐도 지문이 그대로여서 낡은 인덱스가 유효로 판정된다. 형태가 깨진 인덱스(예: `tasks`가 배열이 아님)는 손상으로 보고 정본으로 물러난다. 정확성의 기준은 언제나 무인덱스 경로다 — 인덱스가 없거나 손상되었거나 형식이 다르거나 낡았으면 조용히 정본을 직접 읽고, 결과에 어느 경로로 답했는지를 남긴다. 인덱스 실패가 조회 실패가 되지 않으므로 캐시가 정확성의 조건이 되지 않는다. 유효성 지문은 프로젝트별 worktree의 HEAD와 **커밋되지 않은 변경**, 그리고 공유 이벤트 원장까지 덮는다 — Rundol의 정본은 하나의 Git HEAD가 아니라서 그중 하나라도 빠지면 낡은 인덱스가 유효하다고 판정된다. 인덱스의 내부 키는 문서 고유 식별자이고 번호는 빌드 시점에 해석하므로 나중에 번호를 정리해도 스키마가 남는다. 형식이 바뀌면 마이그레이션 없이 버리고 다시 만든다.
