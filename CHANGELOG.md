...
- **`rdl doc analyze`가 무엇을 봐야 하는지 좁혀 준다.** 분석은 읽을 것을 줄이는 것이지 읽기를 대신하는 것이 아니다 — AI 요약이 원문 읽기를 대체하면 이번에 세운 승인 모델의 취지가 무너진다. 그래서 담긴 것은 전부 기계가 판정할 수 있는 구조적 사실뿐이다. 역참조(이 문서를 무엇이 가리키나), 신뢰 상태, **고아**(아무도 가리키지 않고 태스크도 기능 ID 연결도 없는 문서), **설명 없는 정본**(승인도 연결된 태스크도 없어 왜 이런 내용인지 답할 기록이 없는 문서)를 낸다. 역참조는 저장하지 않고 계산한다 — 저장하면 문서를 고칠 때마다 어긋난다. 고아 판정은 표시 링크만 보지 않는다. 검증 문서는 요구를 참조하지만 화살표가 되돌아오지 않으므로 표시 링크만 보면 모든 TST가 영구히 고아가 되고, 뿌리인 프로젝트 문서도 마찬가지다 — 문서 종류 하나를 통째로 오탐하면 그 신호는 죽는다. 기능 ID를 공유하는 추적성 관계도 연결로 세고 뿌리 문서는 묻지 않는다. 판단이 필요한 품질 평가는 여기가 아니라 `rdl verify`의 렌즈가 맡는다. 이 저장소에 걸어 보니 설명 없는 정본이 34건이다.
- **조회 인덱스가 생겼다 — 삭제 가능한 캐시로, 그리고 기본값이 아닌 선택지로.** `rdl index rebuild`가 문서와 태스크의 스냅숏을 런타임 홈에 만들고 `rdl task list --index`, `rdl context --index`가 그것을 쓴다. **기본 경로는 인덱스가 아니라 정본이다.** 이 저장소에서 재 보니 정본 조회 전체가 4ms인데 인덱스를 써도 되는지 확인하는 데만 259ms가 들었다 — 비용의 전부는 지문을 만드는 Git 하위 프로세스 두 번(`rev-parse` 131ms, `status` 147ms)이고, 인덱스가 아끼려던 문서 읽기는 이미 그보다 훨씬 싸다. 아끼는 것보다 확인하는 것이 비싸면 캐시는 가속이 아니라 감속이므로 기본값으로 두지 않는다. 같은 이유로 증분 갱신도 만들지 않았다 — 갱신을 아무리 싸게 해도 확인 비용이 그대로면 조회는 빨라지지 않는다. 교차점(문서 약 60배 규모)에 이르면 그때 다시 재고 기본값을 뒤집는다. 정확성의 기준은 언제나 무인덱스 경로다 — 인덱스가 없거나 손상되었거나 형식이 다르거나 낡았으면 조용히 정본을 직접 읽고, 결과에 어느 경로로 답했는지를 남긴다. 인덱스 실패가 조회 실패가 되지 않으므로 캐시가 정확성의 조건이 되지 않는다. 유효성 지문은 프로젝트별 worktree의 HEAD와 **커밋되지 않은 변경**, 그리고 공유 이벤트 원장까지 덮는다 — Rundol의 정본은 하나의 Git HEAD가 아니라서 그중 하나라도 빠지면 낡은 인덱스가 유효하다고 판정된다. 인덱스의 내부 키는 문서 고유 식별자이고 번호는 빌드 시점에 해석하므로 나중에 번호를 정리해도 스키마가 남는다. 형식이 바뀌면 마이그레이션 없이 버리고 다시 만든다.
